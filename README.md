...
**The actor never clicks.** An embodied agent under Stagecraft points, walks, speaks, and waits. The human acts. Actuation is not a missing feature; its absence is the trust model, and it is normative (see SPEC.md section 8).
...
Stagecraft is a companion to the Model Context Protocol, not a fork of it. MCP (spec 2026-07-28) carries tools and context; the MCP Apps extension (2026-01-26) renders UI *inside* the chat pane. Stagecraft is the layer for UI *outside* the pane: characters on the desktop and in pages, crossing between them. The Stagecraft verbs bind naturally as MCP tools (section 6.8 of the spec), which makes any MCP client a potential Director, and makes summoning a consented tool call.
